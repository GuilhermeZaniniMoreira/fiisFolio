...
class FiisSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        data = response.xpath("//span[contains(@class,'title') and contains(text(),'Informou distribuição de')]")[0]
        ...
        regex = r'[R$]\s*[.,\d]+|[.,\d]+\s*[R$]'
        amount = re.findall(regex, data.get())

        regexDate = r'[\d]{1,2}/[\d]{1,2}/[\d]{4}'
        dates = re.findall(regexDate, data.get())
