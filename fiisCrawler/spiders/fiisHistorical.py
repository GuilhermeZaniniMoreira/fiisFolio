...
class FiisSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        historicData = response.xpath("//span[contains(@class,'title') and contains(text(),'Informou distribuição de')]")
        
        historic = []
        for distribution in historicData:
            regex = r'[R$]\s*[.,\d]+|[.,\d]+\s*[R$]'
            amount = re.findall(regex, distribution.get())

            regexDate = r'[\d]{1,2}/[\d]{1,2}/[\d]{4}'
            dates = re.findall(regexDate, distribution.get())
            ...
            historic.append(json.dumps(data))
